...
proc update_justified(
  ...
  let
    epochRef =
      dag.getEpochRef(blck, epoch, false).valueOr:
...
proc update_justified(
  ...
  let blck =
    dag.getBlockRef(justified.root).valueOr:
      return
        err ForkChoiceError(kind: fcJustifiedNodeUnknown, blockRoot: justified.root)
...
proc process_block*(
  ...
  for attester_slashing in blck.body.attester_slashings:
    for idx in getValidatorIndices(attester_slashing):
      let i =
        ValidatorIndex.init(idx).valueOr:
          continue
