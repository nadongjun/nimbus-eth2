...
import
  ...
  ../spec/[
    eth2_merkleization, forks, helpers, network, signatures, state_transition, validator
  ],
...
proc handleAttestations(node: BeaconNode, head: BlockRef, slot: Slot) =
  ...
  let
    epochRef =
      node.dag.getEpochRef(attestationHead.blck, slot.epoch, false).valueOr:
    ...
    registeredRes = node.attachedValidators.slashingProtection.withContext:
      ...
      for committee_index in get_committee_indices(committees_per_slot):
        ...
        for index_in_committee, validator_index in committee:
          let validator =
            node.getValidatorForDuties(validator_index, slot).valueOr:
              continue
...
proc handleSyncCommitteeMessages(node: BeaconNode, head: BlockRef, slot: Slot) =
  ...
  for subcommitteeIdx in SyncSubcommitteeIndex:
    for valIdx in syncSubcommittee(syncCommittee, subcommitteeIdx):
      let validator =
        node.getValidatorForDuties(valIdx, slot, slashingSafe = true).valueOr:
          continue
...
proc handleProposal(
  ...
  let
    proposer =
      node.dag.getProposer(head, slot).valueOr:
        return head
    proposerKey = node.dag.validatorKey(proposer).get().toPubKey
    validator =
      node.getValidatorForDuties(proposer, slot).valueOr:
...
proc signAndSendAggregate(
  ...
  try:
    ...
    msg.message.aggregate =
      node.attestationPool[].getAggregatedAttestation(slot, committee_index).valueOr:
        return
...
proc sendAggregatedAttestations(node: BeaconNode, head: BlockRef, slot: Slot) {.async.} =
  ...
  let
    shufflingRef =
      node.dag.getShufflingRef(head, slot.epoch, false).valueOr:
...
proc registerValidators*(node: BeaconNode, epoch: Epoch) {.async.} =
  ...
  for pubkey in node.attachedValidators[].validators.keys:
    let payloadBuilderAddress =
      node.getPayloadBuilderAddress(pubkey).valueOr:
        continue
...
proc updateValidators(node: BeaconNode, validators: openArray[Validator]) =
  ...
  for i in node.dutyValidatorCount .. validators.high:
    let v =
      node.attachedValidators[].getValidator(validators[i].pubkey).valueOr:
        continue
  ...
  for slot in wallSlot ..< wallSlot + SUBNET_SUBSCRIPTION_LEAD_TIME_SLOTS:
    ...
    for committee_index in get_committee_indices(committees_per_slot):
      ...
      for index_in_committee, validator_index in committee:
        let
          validator =
            node.getValidator(validator_index).valueOr:
              continue
