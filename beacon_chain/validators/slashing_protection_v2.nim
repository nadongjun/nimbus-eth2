...
proc setupCachedQueries(db: SlashingProtectionDB_v2) =
  ...
  db.sqlAttMinSourceTargetEpochs =
    db.backend
    .prepareStmt(
      """
    SELECT
  ...
  db.sqlPruneValidatorAttestations =
    db.backend
    .prepareStmt(
      """
    DELETE
  ...
  db.sqlPruneAfterFinalizationBlocks =
    db.backend
    .prepareStmt(
      """
  ...
  db.sqlPruneAfterFinalizationAttestations =
    db.backend
    .prepareStmt(
      """
    WITH
