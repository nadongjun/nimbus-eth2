...
proc removeValidator*(
  ...
  let validator =
    pool.getValidator(publicKey).valueOr:
      return ok(RemoveValidatorStatus.notFound)
  ...
  try:
    ...
    for kind, file in walkDir(validatorsDir):
      if kind == pcDir:
        ...
        let keystore =
          loadKeystore(validatorsDir, secretsDir, keyName, nonInteractive, cache)
          .valueOr:
...
iterator listLoadableKeystores*(config: AnyConf, cache: KeystoreCacheRef): KeystoreData =
...
proc importKeystore*(
  ...
  let
    keypass = KeystorePass.init(password)
    privateKey =
      decryptKeystore(keystore, keypass, cache).valueOr:
...
proc importKeystoreFromFile*(
  ...
  while true:
    ...
    of DecryptionStatus.Success:
      let privateKey =
        ValidatorPrivKey.fromRaw(secret).valueOr:
