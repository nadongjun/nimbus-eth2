...
proc getBlockHeader*(
  ...
  return
    ...
    of 200:
      let response =
        decodeBytes(GetBlockHeaderResponse, resp.data, resp.contentType).valueOr:
    ...
    of 400, 500:
      let error =
        decodeBytes(RestErrorMessage, resp.data, resp.contentType).valueOr:
          let msg =
    ...
    of 200:
      if resp.contentType.isNone() or isWildCard(resp.contentType.get().mediaType):
        ...
        if mediaType == ApplicationJsonMediaType:
          let blck =
            decodeBytes(GetBlockV2Response, resp.data, resp.contentType).valueOr:
              raise newException(RestError, $error)
          some(newClone(blck))
