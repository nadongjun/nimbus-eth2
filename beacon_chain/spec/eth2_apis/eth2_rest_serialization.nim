...
## RestNumeric
proc writeValue*(w: var JsonWriter[RestJson], value: RestNumeric) {.raises: [IOError].} =
...
proc writeValue*(writer: var JsonWriter[RestJson], value: BitSeq) {.raises: [IOError].} =
...
proc writeValue*(writer: var JsonWriter[RestJson], value: BitList) {.raises: [IOError].} =
  ...
  if body.contentType == ApplicationJsonMediaType:
    ...
    let consensusFork =
      ConsensusFork.decodeString(version).valueOr:
        return err(
          RestErrorMessage.init(Http400, UnableDecodeVersionError, [version, $error])
        )
    case consensusFork
    of ConsensusFork.Phase0:
      let blck =
...
proc encodeBytes*[T: EncodeTypes](value: T, contentType: string): RestResult[seq[byte]] =
  case contentType
  of "application/json":
    let data = block:
      try:
...
proc encodeBytes*[T: EncodeArrays](value: T, contentType: string): RestResult[seq[byte]] =
  case contentType
  of "application/json":
    let data = block:
      try:
...
func readSszResBytes(T: typedesc[RestBlockTypes], data: openArray[byte]): RestResult[T] =
  var res: T
  try:
...
proc decodeString*(t: typedesc[EventTopic], value: string): Result[EventTopic, cstring] =
  case value
  of "head":
    ok(EventTopic.Head)
  of "block":
...
proc decodeString*(t: typedesc[StateIdent], value: string): Result[StateIdent, cstring] =
  if len(value) > 2:
    if (value[0] == '0') and (value[1] == 'x'):
...
proc decodeString*(t: typedesc[BlockIdent], value: string): Result[BlockIdent, cstring] =
  if len(value) > 2:
    if (value[0] == '0') and (value[1] == 'x'):
      if len(value) != RootHashSize + 2:
...
proc decodeString*(t: typedesc[Eth2Digest], value: string): Result[Eth2Digest, cstring] =
  if len(value) != RootHashSize + 2:
...
proc readValue*(
  ...
  value =
    decodeString(ValidatorIdent, reader.readValue(string)).valueOr:
  ...
  let
    validatorIds = block:
      ...
      if ids.isSome():
        ...
        for item in ids.get():
          let value =
            decodeString(ValidatorIdent, item).valueOr:
    ...
    filter = block:
      if statuses.isSome():
        ...
        for item in statuses.get():
          let value =
            decodeString(ValidatorFilter, item).valueOr:
              reader.raiseUnexpectedValue($error)
