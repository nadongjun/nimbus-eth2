...
func aggregateAttesters(
  ...
  let firstKey =
    validatorKeys.load(validatorIndices[0]).valueOr:
  ...
  for i in 1 ..< validatorIndices.len:
    let key =
      validatorKeys.load(validatorIndices[i]).valueOr:
        return err("aggregateAttesters: invalid attesting index")
...
proc collectSignatureSets*(
  ...
  let
    ...
    proposer_key =
      validatorKeys.load(proposer_index).valueOr:
  ...
  for i in 0 ..< signed_block.message.body.proposer_slashings.len:
    ...
    block:
      let
        ...
        key =
          validatorKeys.load(header.message.proposer_index).valueOr:
  ...
  for i in 0 ..< signed_block.message.body.attester_slashings.len:
    ...
    block:
      let
        ...
        sig =
          slashing.attestation_1.signature.load().valueOr:
    ...
    block:
      let
        ...
        sig =
          slashing.attestation_2.signature.load().valueOr:
  ...
  for i in 0 ..< signed_block.message.body.voluntary_exits.len:
    ...
    let key =
      validatorKeys.load(volex.message.validator_index).valueOr:
        ...
        when consensusFork >= ConsensusFork.Altair:
          if signed_block.message.body.sync_aggregate.sync_committee_bits.isZeros:
            ...
            sigs.add sync_committee_message_signature_set(
              ...
              beacon_block_root,
              pubkey,
              signed_block.message.body.sync_aggregate.sync_committee_signature
              .load()
              .valueOr do:
                return err("collectSignatureSets: cannot load signature")
              ,
