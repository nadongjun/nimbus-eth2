...
proc serveSyncCommitteeMessage*(
  ...
  let
    ...
    validator =
      vc.getValidatorForDuties(duty.pubkey, slot, slashingSafe = true).valueOr:
        return false
    message = block:
...
proc produceAndPublishSyncCommitteeMessages(
  ...
  let statistics = block:
    ...
    try:
      ...
      let pending =
        pendingSyncCommitteeMessages.filterIt(not (it.finished())).mapIt(
          it.cancelAndWait()
        )
...
proc produceAndPublishContributions(
  ...
  var (contributions, pendingFutures, contributionsMap) = block:
    ...
    for duty in duties:
      let validator =
        vc.getValidatorForDuties(duty.pubkey, slot).valueOr:
          continue
      ...
      for inindex in duty.validator_sync_committee_indices:
        let
          ...
          signature =
            vc.getSyncCommitteeSelectionProof(duty.pubkey, slot, inindex).valueOr:
              continue
