...
proc prepareRandao(
  ...
  let
    ...
    currentSlot = (await vc.checkedWaitForSlot(destSlot, destOffset, false)).valueOr:
      ...
      return
    validator =
      vc.getValidatorForDuties(proposerKey, slot, true).valueOr:
        return
...
proc proposeBlock(
  ...
  let validator =
    vc.getValidatorForDuties(proposerKey, slot).valueOr:
      return
...
proc pollForEvents(
  ...
  while true:
    ...
    for event in events:
      ...
      of "data":
        let blck =
          EventBeaconBlockObject.decodeString(event.data).valueOr:
...
proc runBlockEventMonitor(service: BlockServiceRef, node: BeaconNodeServerRef) {.async.} =
  let
    vc = service.client
...
proc runBlockPollMonitor(service: BlockServiceRef, node: BeaconNodeServerRef) {.async.} =
  let
    vc = service.client
    roles = {BeaconNodeRole.BlockProposalData}
    statuses = {RestBeaconNodeStatus.Synced}
