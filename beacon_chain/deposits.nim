...
proc restValidatorExit(config: BeaconNodeConf) {.async.} =
  let
    client =
      RestClientRef.new(config.restUrlForExit).valueOr:
  ...
  if config.exitAllValidatorsFlag:
    ...
    for pubKey in config.exitedValidators:
      let validatorStorage =
        decryptor.getValidator(pubkey).valueOr:
  ...
  for validator in validators:
    ...
    if config.printData:
      let bytes =
        encodeBytes(signedExit, "application/json").valueOr:
