...
proc getRandomNetKeys*(rng: var HmacDrbgContext): NetKeyPair =
  let privKey =
    PrivateKey.random(Secp256k1, rng).valueOr:
...
proc getPersistentNetKeys*(
  ...
  if netKeyFile == "random":
    let
      ...
      pres =
        PeerId.init(keys.pubkey).valueOr:
    ...
    if fileAccessible(keyPath, {AccessFlags.Find}) and allowLoadExisting:
      ...
      let
        privKey =
          loadNetKeystore(keyPath, insecurePassword).valueOr:
...
proc getPersistentNetKeys*(rng: var HmacDrbgContext, config: BeaconNodeConf): NetKeyPair =
  case config.cmd
...
proc broadcast(node: Eth2Node, topic: string, msg: auto): Future[Result[void, cstring]] =
