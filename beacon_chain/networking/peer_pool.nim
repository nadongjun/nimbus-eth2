...
proc waitNotEmptyEvent[A, B](pool: PeerPool[A, B], filter: set[PeerType]): Future[void] =
...
proc addPeerNoWait*[A, B](pool: PeerPool[A, B], peer: A, peerType: PeerType): PeerStatus =
...
proc setScoreCheck*[A, B](pool: PeerPool[A, B], scoreCheckCb: PeerScoreCheckCallback[A]) =
  ## Sets ScoreCheck callback.
  pool.scoreCheck = scoreCheckCb

proc setOnDeletePeer*[A, B](pool: PeerPool[A, B], deletePeerCb: PeerOnDeleteCallback[A]) =
