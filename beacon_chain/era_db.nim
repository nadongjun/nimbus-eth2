...
proc getBlockSSZ*(f: EraFile, slot: Slot, bytes: var seq[byte]): Result[void, string] =
  ...
  let len =
    uncompressedLenFramed(tmp).valueOr:
  ...
  discard
    uncompressFramed(tmp, bytes, checkIntegrity = false).valueOr:
...
proc getEraFile(
  ...
  let
    eraRoot =
      eraRoot(db.genesis_validators_root, historical_roots, historical_summaries, era)
      .valueOr:
