...
func validatorKey*(dag: ChainDAGRef, index: ValidatorIndex or uint64): Opt[CookedPubKey] =
...
func isCanonical*(dag: ChainDAGRef, bid: BlockId): bool =
  ...
  ## `dag.head`.
  let current =
    dag.getBlockIdAtSlot(bid.slot).valueOr:
...
func epochKey(dag: ChainDAGRef, bid: BlockId, epoch: Epoch): Opt[EpochKey] =
  ...
  let bsi =
    dag.epochAncestor(bid, epoch).valueOr:
...
func init*(
  ...
  let
    ...
    shufflingRef =
      dag.findShufflingRef(state.latest_block_id, epoch).valueOr:
    ...
    epochRef = EpochRef(
      key:
        dag.epochKey(state.latest_block_id, epoch).expect(
...
func loadStateCache(dag: ChainDAGRef, cache: var StateCache, bid: BlockId, epoch: Epoch) =
...
proc getState(
  ...
  let state_root =
    db.getStateRoot(block_root, slot).valueOr:
      return false
...
proc getStateByParent(
  ...
  let
    summary =
      dag.db.getBeaconBlockSummary(bid.root).valueOr:
        return false
    parentMinSlot =
      dag.db
...
proc applyBlock(
  ...
  of ConsensusFork.Phase0:
    let data =
      getBlock(dag, bid, phase0.TrustedSignedBeaconBlock).valueOr:
  ...
  of ConsensusFork.Altair:
    let data =
      getBlock(dag, bid, altair.TrustedSignedBeaconBlock).valueOr:
        return err("Block load failed")
    state_transition(
      dag.cfg, state, data, cache, info, dag.updateFlags + {slotProcessed}, noRollback
    )
  of ConsensusFork.Bellatrix:
    let data =
      getBlock(dag, bid, bellatrix.TrustedSignedBeaconBlock).valueOr:
        return err("Block load failed")
    state_transition(
      dag.cfg, state, data, cache, info, dag.updateFlags + {slotProcessed}, noRollback
    )
  of ConsensusFork.Capella:
    let data =
      getBlock(dag, bid, capella.TrustedSignedBeaconBlock).valueOr:
        return err("Block load failed")
    state_transition(
      dag.cfg, state, data, cache, info, dag.updateFlags + {slotProcessed}, noRollback
    )
  of ConsensusFork.Deneb:
    let data =
      getBlock(dag, bid, deneb.TrustedSignedBeaconBlock).valueOr:
        return err("Block load failed")
    state_transition(
      dag.cfg, state, data, cache, info, dag.updateFlags + {slotProcessed}, noRollback
    )
...
proc getShufflingRef*(
  ...
  # epoch match
  let epochRef =
    dag.getEpochRef(blck, epoch, preFinalized).valueOr:
  ...
  dag.db.withManyWrites:
    ...
    if blockHorizon > GENESIS_SLOT and blockHorizon <= dag.tail.slot:
      var
        ...
        cur =
          dag.getBlockIdAtSlot(blockHorizon - 1).map(
            proc(x: auto): auto =
              x.bid
          )
...
proc loadExecutionBlockHash*(dag: ChainDAGRef, bid: BlockId): Eth2Digest =
  let blockData =
    dag.getForkedBlock(bid).valueOr:
      return ZERO_HASH
...
proc getProposer*(dag: ChainDAGRef, head: BlockRef, slot: Slot): Opt[ValidatorIndex] =
  let
    epochRef =
      dag.getEpochRef(head.bid, slot.epoch(), false).valueOr:
...
func aggregateAll*(
  ...
  let firstKey =
    dag.validatorKey(validator_indices[0]).valueOr:
  ...
  for i in 1 ..< validator_indices.len:
    let key =
      dag.validatorKey(validator_indices[i]).valueOr:
        return err("aggregate: invalid validator index")
...
proc rebuildIndex*(dag: ChainDAGRef) =
  ...
  for i, state_root in canonical.mpairs():
    ...
    for slot in startSlot ..< startSlot + (EPOCHS_PER_STATE_SNAPSHOT * SLOTS_PER_EPOCH):
      let bids =
        dag.getBlockIdAtSlot(slot).valueOr:
