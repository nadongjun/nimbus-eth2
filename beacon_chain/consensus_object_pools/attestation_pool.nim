    ...
    let
      ...
      status =
        if i < (blocks.len - ForkChoiceHorizon) and (i mod 1024 != 0):
          ...
          epochRef =
            dag.getEpochRef(blckRef, blckRef.slot.epoch, false).expect(
              "Getting an EpochRef should always work for non-finalized blocks"
            )
          let
            blck =
              dag.getForkedBlock(blckRef.bid).expect(
...
proc check_attestation_compatible*(
  ...
  let
    ...
    attestedBlck =
      dag.getBlockRef(attestation.data.target.root).valueOr:
        return err("Unknown `target.root`")
    dependentSlot = targetEpoch.attester_dependent_slot
    dependentBid =
      dag.atSlot(attestedBlck.bid, dependentSlot).valueOr:
...
proc selectOptimisticHead*(
  ...
  let headBlock =
    pool.dag.getBlockRef(newHeadRoot.get()).valueOr:
