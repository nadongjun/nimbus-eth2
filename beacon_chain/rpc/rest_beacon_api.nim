...
proc installBeaconApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodGet, "/eth/v1/beacon/deposit_snapshot") do() -> RestApiResponse:
    let snapshot =
      node.db.getDepositTreeSnapshot().valueOr:
  ...
  router.api(MethodGet, "/eth/v1/beacon/states/{state_id}/root") do(
    ...
    let
      ...
      bslot =
        node.getBlockSlotId(sid).valueOr:
  ...
  proc getIndices(
    ...
    for item in validatorIds:
      ...
      of ValidatorQueryKind.Index:
        let vindex =
          item.index.toValidatorIndex().valueOr:
            case error
            of ValidatorIndexError.TooHighValue:
              return
                err(RestErrorMessage.init(Http400, TooHighValidatorIndexValueError))
  ...
  proc getValidators(
    ...
    node.withStateForBlockSlotId(bslot):
      let
        ...
        indices =
          node.getIndices(validatorIds, state).valueOr:
            ...
            if len(validatorIds) == 0:
              ...
              for index, validator in getStateField(state, validators):
                let
                  ...
                  status =
                    validator.getStatus(stateEpoch).valueOr:
  ...
  router.api(MethodGet, "/eth/v1/beacon/headers") do(
    ...
    let bdata =
      node.getForkedBlock(BlockIdent.init(qslot)).valueOr:
  ...
  router.api(MethodGet, "/eth/v1/beacon/headers/{block_id}") do(
    ...
    let
      ...
      bdata =
        node.getForkedBlock(bid).valueOr:
          return RestApiResponse.jsonError(Http404, BlockNotFoundError)
  ...
  router.api(MethodPost, "/eth/v1/beacon/blocks") do(
    ...
    let res = block:
      ...
      var
        restBlock =
          decodeBody(RestPublishedSignedBlockContents, body, version).valueOr:
            return RestApiResponse.jsonError(error)
  ...
  router.api(MethodPost, "/eth/v2/beacon/blocks") do(
    ...
    let res = block:
      ...
      var
        restBlock =
          decodeBodyJsonOrSsz(RestPublishedSignedBlockContents, body, version).valueOr:
            return RestApiResponse.jsonError(error)
        forked = ForkedSignedBeaconBlock.init(restBlock)
  ...
  router.api(MethodPost, "/eth/v1/beacon/blinded_blocks") do(
    ...
    withConsensusFork(currentEpochFork):
      when consensusFork >= ConsensusFork.Capella:
        let
          restBlock =
            decodeBodyJsonOrSsz(consensusFork.SignedBlindedBeaconBlock, body).valueOr:
              return RestApiResponse.jsonError(error)
          payloadBuilderClient =
            node.getPayloadBuilderClient(restBlock.message.proposer_index).valueOr:
              return RestApiResponse.jsonError(
        ...
        var
          restBlock =
            decodeBody(RestPublishedSignedBeaconBlock, body, version).valueOr:
              return RestApiResponse.jsonError(error)
  ...
  router.api(MethodGet, "/eth/v2/beacon/blocks/{block_id}") do(
    ...
    let
      blockIdent = block_id.valueOr:
        return RestApiResponse.jsonError(Http400, InvalidBlockIdValueError, $error)
      bid =
        node.getBlockId(blockIdent).valueOr:
          return RestApiResponse.jsonError(Http404, BlockNotFoundError)
  ...
  router.api(MethodGet, "/eth/v1/beacon/blocks/{block_id}/attestations") do(
    ...
    let
      blockIdent = block_id.valueOr:
        return RestApiResponse.jsonError(Http400, InvalidBlockIdValueError, $error)
      bdata =
        node.getForkedBlock(blockIdent).valueOr:
          return RestApiResponse.jsonError(Http404, BlockNotFoundError)
