...
proc installValidatorApiHandlers*(router: var RestRouter, node: BeaconNode) =
  ...
  router.api(MethodPost, "/eth/v1/validator/duties/attester/{epoch}") do(
    ...
    let qhead = block:
      ...
      res.get()
    let shufflingRef =
      node.dag.getShufflingRef(qhead, qepoch, true).valueOr:
  ...
  router.api(MethodGet, "/eth/v1/validator/duties/proposer/{epoch}") do(
    ...
    let qhead = block:
      ...
      res.get()
    let epochRef =
      node.dag.getEpochRef(qhead, qepoch, true).valueOr:
  ...
  router.api(MethodPost, "/eth/v1/validator/duties/sync/{epoch}") do(
    ...
    if qSyncPeriod == headSyncPeriod:
      ...
      let bsi =
        node.dag.getBlockIdAtSlot(earliestSlotInQSyncPeriod).valueOr:
  ...
  router.api(MethodGet, "/eth/v2/validator/blocks/{slot}") do(
    ...
    let contentType =
      preferredContentType(jsonMediaType, sszMediaType).valueOr:
    ...
    let message = block:
      ...
      let qhead = block:
        ...
        tres
      let proposer =
        node.dag.getProposer(qhead, qslot).valueOr:
  ...
  router.api(MethodGet, "/eth/v1/validator/attestation_data") do(
    ...
    let adata = block:
      ...
      let qhead = block:
        ...
        tres
      let epochRef =
        node.dag.getEpochRef(qhead, qslot.epoch, true).valueOr:
          return RestApiResponse.jsonError(Http400, PrunedStateError, $error)
    ...
    template getAndCacheShufflingRef(
      ...
      if shufflingRefVar.isNone:
        shufflingRefVar = block:
          let tmp =
            node.dag.getShufflingRef(head, epoch, true).valueOr:
              return RestApiResponse.jsonError(Http400, PrunedStateError)
          Opt.some tmp
  ...
  router.api(MethodGet, "/eth/v1/validator/sync_committee_contribution") do(
    ...
    let
      blck =
        node.dag.getBlockRef(qroot).valueOr:
