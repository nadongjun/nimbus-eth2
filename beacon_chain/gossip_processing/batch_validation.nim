...
proc scheduleAggregateChecks*(
  ...
  let
    ...
    aggregatorSig =
      signedAggregateAndProof.signature.load().valueOr:
        return err("aggregateAndProof: invalid proof signature")
    slotSig =
      aggregate_and_proof.selection_proof.load().valueOr:
...
proc scheduleContributionChecks*(
  ...
  let
    ...
    aggregatorSig =
      signedContributionAndProof.signature.load().valueOr:
        return err("SignedContributionAndProof: invalid proof signature")
    proofSig =
      contribution_and_proof.selection_proof.load().valueOr:
        return err("SignedContributionAndProof: invalid selection signature")
    contributionSig =
      contribution.signature.load().valueOr:
...
proc scheduleBlsToExecutionChangeCheck*(
  ...
  let
    ...
    sig =
      signedBLSToExecutionChange.signature.load().valueOr:
        return
