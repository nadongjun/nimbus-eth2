...
func check_beacon_and_target_block(
  ...
  let blck =
    pool.dag.getBlockRef(data.beacon_block_root).valueOr:
  ...
  let target =
    blck.atCheckpoint(data.target).valueOr:
...
proc validateBlobSidecar*(
  ...
  let parent =
    dag.getBlockRef(block_header.parent_root).valueOr:
  ...
  let proposer =
    getProposer(dag, parent, block_header.slot).valueOr:
...
proc validateBeaconBlock*(
  ...
  # passes validation.
  let parent =
    dag.getBlockRef(signed_beacon_block.message.parent_root).valueOr:
  ...
  let proposer =
    getProposer(dag, parent, signed_beacon_block.message.slot).valueOr:
...
proc validateAttestation*(
  ...
  let pubkey =
    pool.dag.validatorKey(validator_index).valueOr:
...
proc validateSyncCommitteeMessage*(
  ...
  let
    ...
    blck =
      dag.getBlockRef(blockRoot).valueOr:
  ...
  let senderPubKey =
    dag.validatorKey(msg.validator_index).valueOr:
...
proc validateContribution*(
  ...
  let subcommitteeIdx =
    SyncSubcommitteeIndex.init(msg.message.contribution.subcommittee_index).valueOr:
